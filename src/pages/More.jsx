...
//
// FIX (Phase 9 Bug 1):
//   Added `Bell` / `reminders` to icon mapping.
//
// FIX (Phase 11 — dual ICON_MAP consolidation):
//   Replaced the local ICON_MAP constant with a shared import from
//   src/lib/iconMap.js. Sidebar.jsx has been updated identically.
//   Adding a new More item to rbac.js now only requires editing iconMap.js.

import { useNavigate }     from "react-router-dom";
import { Settings }        from "lucide-react"; // fallback only
  ...
  return (
    <AppShell pageTitle={t("nav.more")}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 mb-8">
        {items.map((item) => {
          // Resolve icon: look up item.icon in the shared ICON_MAP, fall back to Settings.
          const Icon = ICON_MAP[item.icon] || ICON_MAP[item.id] || Settings;
          return (
            <button
              key={item.id}
