 ...
 *
 * BUG 2 FIX — handleSubmit:
 *   Previously logAudit() was called with a bare `await` OUTSIDE of the
 *   try/catch block. If the audit write failed (bad network, Firestore rules,
 *   quota exceeded, etc.) it threw an unhandled exception AFTER the customer
 *   was already saved — leaving the user on an error screen with no way to
 *   know whether their data was saved or not.
 *
 *   Fix:
 *   1. logAudit() is now called INSIDE the existing try/catch block so any
 *      write failure is caught and assigned to saveError for display.
 *   2. The audit write is wrapped in its OWN inner try/catch. This is
 *      intentional: an audit failure is non-critical and must never block
 *      or roll back a successful customer save. The customer is saved first;
 *      if the audit write then fails, the user is navigated to the customer
 *      page as normal, and the audit error is silently logged to the console
 *      so it can be investigated without disrupting the user's workflow.
 */
...
export default function CustomerForm({ mode = 'create' }) {
  ...
  // ── Submit ─────────────────────────────────────────────────────────────────
  // BUG 2 FIX: logAudit is now inside the main try/catch, and wrapped in its
  // own inner try/catch. The customer save always completes first. If the
  // subsequent audit write fails, it logs to console but does NOT surface an
  // error to the user and does NOT prevent navigation — because an audit log
  // failure is an internal logging concern, not a user-facing failure.
  const handleSubmit = async () => {
    ...
    try {
      if (mode === 'create') {
        // 1. Save customer — this must succeed before anything else.
        const newId = await createCustomer({ ...data, createdBy: uid });

        // 2. Clear draft immediately after successful save.
        localStorage.removeItem(DRAFT_KEY);

        // 3. Attempt audit log — failure here must never affect the user.
        try {
          await logAudit({
          ...
          // Non-critical: log for developer inspection, do not surface to user.
          console.warn('[CustomerForm] Audit log write failed (non-critical):', auditErr);
        }

        // 4. Navigate to the new customer's detail page.
        navigate(`/customers/${newId}`);

      } else {
        // 1. Save updated customer.
        await updateCustomer(id, data);

        // 2. Attempt audit log — failure must not affect the user.
        try {
          await logAudit({
        ...
        // 3. Navigate back to the customer detail page.
        navigate(`/customers/${id}`);
      }

    } catch (err) {
      // This catch handles failures in createCustomer / updateCustomer only.
      // The user sees a clear error message and stays on the form.
