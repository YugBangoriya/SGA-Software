...
// Each template: name + body in English, Hindi, Gujarati.
...
const EMPTY_FORM = { name: "", bodyEn: "", bodyHi: "", bodyGu: "" };
const LANG_LABELS = {
  bodyEn: { label: "English", flag: "🇬🇧", placeholder: "Hi {customerName}, your CNG kit from Shree Ganesh Automobile is due for a follow-up. Please call us at your convenience." },
  bodyHi: { label: "Hindi", flag: "🇮🇳", placeholder: "नमस्ते {customerName}, आपका CNG किट का फॉलो-अप बाकी है। कृपया हमसे संपर्क करें।" },
  bodyGu: { label: "Gujarati", flag: "🏵️", placeholder: "નમસ્તે {customerName}, તમારા CNG કીટ માટે ફૉલો-અપ બાકી છે. કૃપા કરી અમારો સંપર્ક કરો." },
};
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 600, color: T.textPrimary }}>{template.name}</div>
          <div style={{ fontSize: 11, color: T.textMeta, marginTop: 2 }}>
            {template.bodyEn ? template.bodyEn.substring(0, 60) + (template.bodyEn.length > 60 ? "…" : "") : "No English body"}
          </div>
        </div>
      ...
      {open && (
        <div style={{ borderTop: `1px solid ${T.border}`, padding: "14px 16px" }}>
          ...
          >
            💡 Available variables: <code>{"{customerName}"}</code>, <code>{"{vehicleNo}"}</code>, <code>{"{installDate}"}</code>
          </div>
        </div>
      )}
    </div>
...
export default function FollowUpTemplates() {
  ...
  const [activeTab, setActiveTab] = useState("bodyEn");
  ...
  const handleSave = async () => {
    ...
    if (!form.bodyEn.trim()) { setError("English message body is required."); return; }
    setSaving(true);
    setError("");
    try {
      const id = await saveFollowUpTemplate(editingId ? { ...form, id: editingId } : form);
  ...
  return (
    <>
      ...
      >
        ...
        >
          Available variables in message bodies: <code>{"{customerName}"}</code>, <code>{"{vehicleNo}"}</code>, <code>{"{installDate}"}</code>.
          These are replaced with real customer data when the message is sent.
        </div>

        {followUpTemplates.length > 0 ? (
          followUpTemplates.map((t) => (
            <TemplateCard key={t.id} template={t} onEdit={openEdit} onDelete={setDeleteTarget} />
          ))
        ) : (
        ...
        >
          ...
          >
              ...
              {Object.entries(LANG_LABELS).map(([key, meta]) => (
                <button
                  ...
                  style={{
                    ...
                    cursor: "pointer",
                    border: "none",
                    borderRight: key !== "bodyGu" ? `1px solid ${T.border}` : "none",
            ...
            <FieldRow
              ...
              required={activeTab === "bodyEn"}
              hint={activeTab === "bodyEn" ? "Required" : "Optional"}
            >
              <Textarea
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
              <Button variant="ghost" onClick={() => { setShowForm(false); setForm(EMPTY_FORM); setError(""); }}>
                Cancel
              </Button>
