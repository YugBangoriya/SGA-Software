// SGA — Last updated: Removed dead `@react-pdf/renderer` and `react` dynamic imports
// that caused SyntaxError: Unexpected token '<' during Firebase deployment.
// These imports were never actually used — generateSimplePDF (using pdfkit) was
// always the active code path. Cleaned up the try/catch to call generateSimplePDF
// directly. Added pdfkit to functions/package.json as it is required here.
// No business logic or behaviour has changed.
// ============================================================
...
// 3. Generates PDF server-side using pdfkit (pure Node.js)
// 4. Uploads PDF buffer to WhatsApp Media endpoint → gets media ID
// 5. Sends the PDF document message via WhatsApp Cloud API
// 6. Fallback: if media upload fails, uploads to Firebase Storage
//    and sends a text message with a download link instead
...
// ── Upload PDF buffer to Firebase Storage (used as fallback) ──
...
async function sendWhatsAppDocument(phone, mediaId, invoiceNo, customerName, accessToken, phoneNumberId) {
  ...
  const response = await axios.post(waUrl(phoneNumberId), payload, {
    headers: {
      Authorization: `Bearer ${accessToken}`,
...
exports.sendInvoiceWhatsApp = onCall(
  ...
  async (request) => {
    ...
    if (!invoiceId || !phone) {
      throw new HttpsError("invalid-argument", "invoiceId and phone are required.");
    }
    ...
    // ── Generate PDF using pdfkit (pure Node.js) ──────
    // pdfkit is a server-side PDF library — no React, no JSX, no browser APIs.
    // The @react-pdf/renderer import that was previously here was dead code:
    // it was imported but never actually used, and it caused a SyntaxError
    // during deployment because @react-pdf/renderer contains JSX internally.
    let pdfBuffer;
    try {
      pdfBuffer = await generateInvoicePDF(invoice, businessSettings);
    } catch (pdfErr) {
    ...
    try {
      ...
      // Fallback: upload to Firebase Storage and send as text link
      try {
        const { url, fileName } = await uploadPDFToStorage(pdfBuffer, invoice.invoiceNo);
        await sendWhatsAppTextWithLink(formattedPhone, invoice, url, accessToken, phoneNumberId);
        await logWhatsAppAudit(invoiceId, invoice, formattedPhone, request.auth, "sent_as_link_fallback");

        // Clean up temp file after 1 hour
        setTimeout(async () => {
          try { await storage.bucket().file(fileName).delete(); } catch {}
        }, 60 * 60 * 1000);

        return { success: true, method: "link", invoiceNo: invoice.invoiceNo };
      } catch (fallbackErr) {
        throw new HttpsError("internal", "WhatsApp send failed: " + fallbackErr.message);
      }
    }
    ...
    try {
      ...
      await logWhatsAppAudit(invoiceId, invoice, formattedPhone, request.auth, "sent_as_document");
...
// ── Server-side PDF generator (pdfkit — pure Node.js) ────
// pdfkit must be in functions/package.json dependencies.
// Generates a clean, professional A4 invoice PDF with:
//   business header, invoice number, customer + vehicle details,
//   itemised table, totals, GST (conditional), payment info, terms.
async function generateInvoicePDF(invoice, businessSettings) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (businessSettings.address) doc.text(businessSettings.address, 50, 88);
    if (businessSettings.phone)   doc.text(`Ph: ${businessSettings.phone}`, 50, 100);
    if (invoice.gstEnabled && businessSettings.gstNumber) {
      doc.text(`GSTIN: ${businessSettings.gstNumber}`, 50, 112);
    }

    // Invoice title (right-aligned)
    doc.fillColor(C.primary).fontSize(28).font("Helvetica-Bold")
       .text("INVOICE", 350, 50, { align: "right" });
    doc.fillColor(C.text).fontSize(11).font("Courier-Bold")
    ...
    // ── Divider ───────────────────────────────────────
    doc.moveTo(50, 132).lineTo(545, 132).strokeColor(C.primary).lineWidth(2).stroke();
    let y = 148;
    ...
    doc.fontSize(9).font("Helvetica").fillColor(C.muted)
       .text(`Ph: ${customer.phone || "—"}`, 50, y + 14);
    doc.fillColor(C.text).fontSize(11).font("Courier-Bold")
       .text(vehicle.registrationNo || "—", 310, y);
    ...
    doc.font("Courier").fillColor(C.text)
       .text(`Rs.${parseFloat(invoice.subtotal || 0).toFixed(2)}`, valX, y, { width: 55, align: "right" });
    y += 14;

    if (invoice.gstEnabled) {
      doc.fillColor(C.muted).font("Helvetica").text("CGST (9%):", totalsX, y);
      doc.font("Courier").fillColor(C.text)
         .text(`Rs.${parseFloat(invoice.cgst || 0).toFixed(2)}`, valX, y, { width: 55, align: "right" });
      y += 14;
      doc.fillColor(C.muted).font("Helvetica").text("SGST (9%):", totalsX, y);
      doc.font("Courier").fillColor(C.text)
         .text(`Rs.${parseFloat(invoice.sgst || 0).toFixed(2)}`, valX, y, { width: 55, align: "right" });
      y += 14;
    }
    ...
    doc.font("Courier-Bold")
       .text(`Rs.${totalAmount.toFixed(2)}`, valX, y, { width: 55, align: "right" });
    y += 18;

    doc.fillColor("#1A7A1A").fontSize(9).font("Helvetica").text("Amount Paid:", totalsX, y);
    doc.font("Courier")
       .text(`Rs.${amountPaid.toFixed(2)}`, valX, y, { width: 55, align: "right" });
    y += 14;

    doc.fillColor(balanceDue > 0 ? "#CC0000" : "#1A7A1A")
       .fontSize(10).font("Helvetica-Bold").text("Balance Due:", totalsX, y);
    doc.font("Courier-Bold")
       .text(`Rs.${balanceDue.toFixed(2)}`, valX, y, { width: 55, align: "right" });
    y += 24;
    ...
    y += 12;
    const terms =
      businessSettings.termsAndConditions ||
